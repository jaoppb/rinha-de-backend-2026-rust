...
fn main() {
    ...
    let mcc_data: HashMap<String, f32> = serde_json::from_reader(mcc_file).expect("failed to parse mcc_risk.json");
    ...
    let norm_data: Normalization = serde_json::from_reader(norm_file).expect("failed to parse normalization.json");

    // Write as static arrays/structs to the generated file
    writeln!(f, "pub static MCC_RISKS: [f32; 65536] = {:?};", mcc_array).unwrap();
    writeln!(f, "pub static NORMALIZATION: Normalization = Normalization {{").unwrap();
    writeln!(f, "    max_amount: {:?},", norm_data.max_amount).unwrap();
    writeln!(f, "    max_installments: {:?},", norm_data.max_installments).unwrap();
    writeln!(f, "    amount_vs_avg_ratio: {:?},", norm_data.amount_vs_avg_ratio).unwrap();
    ...
    writeln!(f, "    max_merchant_avg_amount: {:?},", norm_data.max_merchant_avg_amount).unwrap();
    writeln!(f, "}};").unwrap();
}
